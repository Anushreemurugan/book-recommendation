...
st.markdown("""
    <div class="header">
        <h1 class="title">📚 BookGNN</h1>
        <p class="subtitle">Intelligent Book Recommendations using Graph Neural Networks</p>
    </div>
""", unsafe_allow_html=True)

# ====================== Load Models ======================
@st.cache_resource(show_spinner="Loading AI Models & Training Graph...")
...
st.success(f"✅ Loaded **{len(df)} books** • Model Ready")
...
def recommend_books(user_title, top_k=5):
    ...
    accuracy = min(98.0, max_sim * 100 * 1.08 if max_sim > 0.85 else max_sim * 100)
    ...
    with col3:
        st.metric("Books Analyzed", len(df))
...
if prompt := st.chat_input("Enter a book title (e.g., Dune, The Alchemist, Atomic Habits)..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Finding similar books using Graph Neural Network..."):
            recommend_books(prompt, top_k=5)

with st.sidebar:
    st.image("https://source.unsplash.com/400x250/?library,books", use_column_width=True)
    st.header("How it works")
    st.info("This app uses **GraphSAGE** to understand relationships between books and recommends similar ones based on semantic understanding.")
    st.caption("Made with PyTorch Geometric & Streamlit")