...
# Clean & Modern Styling (Light Background)
st.markdown("""
    <style>
    .main {
...
def recommend_books(user_title, top_k=5):
    ...
    if not match.empty:
        ...
        st.info(f"🔍 Computing for **{user_title}**...")
    ...
    with col1: st.metric("Best Similarity", f"{max_sim:.3f}")
    with col2: st.metric("Accuracy", f"{accuracy:.1f}%")
    with col3: st.metric("Books in Database", len(df))
    ...
    for i in top_indices:
        ...
        <div class="rec-card">
            ...
            <p><strong>Similarity Score:</strong> {sim_score:.3f}</p>
            <p>{summary_text}</p>
        </div>
        """, unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.info("This recommender uses **Graph Neural Networks (GraphSAGE)** to understand deep relationships between books.")
