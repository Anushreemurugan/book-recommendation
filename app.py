...
st.markdown("""
    ...
    .title { font-size: 46px; font-weight: 700; margin-bottom: 10px; }
    .subtitle { font-size: 21px; opacity: 0.95; }
    .rec-card {
        background: white;
        padding: 22px;
        border-radius: 16px;
...
st.markdown("""
    <div class="header">
        ...
        <p class="subtitle">Discover your next favorite book using Graph Neural Networks</p>
    </div>
""", unsafe_allow_html=True)
...
def get_full_story_summary(title):
    ...
    if not full or len(full.strip()) < 30:
        return None  # Return None if no good summary
    cleaned = re.sub(r'\s+', ' ', full).strip()
    return textwrap.fill(cleaned, width=100)

# ====================== Improved Recommendation with Fallback ======================
def recommend_books(user_title, top_k=5):
    ...
    top_indices = similarities.argsort()[-top_k-15:][::-1]   # Extra candidates for fallback

    # Collect recommendations with valid summaries
    recommendations = []
    fallback_books = []
    ...
    for i in top_indices:
        ...
        if summary_text:  # Has valid summary
    ...
    # If we don't have enough good summaries, add fallback books
...
with st.sidebar:
    ...
    st.info("This recommender uses **GraphSAGE** to understand book relationships and always tries to show books with available summaries.")
